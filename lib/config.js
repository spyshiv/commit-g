...
export async function loadConfig() {
  ...
  const configPaths = [
    path.join(process.cwd(), ".aicommitrc"),
    path.join(process.cwd(), ".aicommitrc.json"),
    path.join(process.cwd(), ".aicommitrc.js"),
    path.join(process.cwd(), "aicommit.config.js"),
  ...
  if (!defaultConfig.apiKey) {
    throw new Error(
      "No API key provided. Please set GEMINI_API_KEY environment variable " +
        "or create a configuration file (.aicommitrc, .aicommitrc.json, or .aicommitrc.js)"
    );
  }
